...
import ee.sk.siddemo.services.SmartIdNotificationBasedSigningService;
import jakarta.servlet.http.HttpSession;

@Controller
public class SmartIdNotificationBasedSigningController {

    private final Logger logger = LoggerFactory.getLogger(SmartIdNotificationBasedSigningController.class);

    private final SmartIdNotificationBasedSigningService smartIdNotificationBasedSigningService;

    public SmartIdNotificationBasedSigningController(SmartIdNotificationBasedSigningService smartIdNotificationBasedSigningService) {
        this.smartIdNotificationBasedSigningService = smartIdNotificationBasedSigningService;
    }

    @PostMapping(value = "notification-based/start-signing-with-document-number")
    public ModelAndView sendNotificationSigningRequestWithDocumentNumber(@ModelAttribute("userDocumentNumberRequest") UserDocumentNumberRequest userDocumentNumberRequest,
        ...
        String verificationCode = smartIdNotificationBasedSigningService.startSigningWithDocumentNumber(session, userDocumentNumberRequest);
        model.addAttribute("verificationCode", verificationCode);
        return new ModelAndView("notification-based/signing", model);
    }

    @PostMapping(value = "notification-based/start-signing-with-person-code")
    public ModelAndView sendNotificationSigningRequestWithPersonCode(@ModelAttribute("userRequest") UserRequest userRequest,
        ...
        String verificationCode = smartIdNotificationBasedSigningService.startSigningWithPersonCode(session, userRequest);
        model.addAttribute("verificationCode", verificationCode);
        return new ModelAndView("notification-based/signing", model);
    }

    @GetMapping(value = "notification-based/check-signing-status")
    @ResponseBody
    public ResponseEntity<?> checkNotificationSigningStatus(HttpSession session) {
        try {
            smartIdNotificationBasedSigningService.checkSignatureStatus(session);
